...
class TicketableTicket extends Model implements Sortable
{
    ...
    {
        $this->setTable(config('yanselmask.bookings.tables.ticketable_tickets'));
        $this->mergeRules([
            'ticketable_id' => 'required|integer',
            'ticketable_type' => 'required|string|strip_tags|max:150',
            'slug' => 'required|alpha_dash|max:150|unique:'.config('yanselmask.bookings.tables.ticketable_tickets').',slug,NULL,id,ticketable_id,'.($this->ticketable_id ?? 'null').',ticketable_type,'.($this->ticketable_type ?? 'null'),
